<?php
// Create bill form (updated)
?>
<?= $this->extend('layout') ?>

<?= $this->section('content') ?>
<div style="max-width: 600px; margin: 0 auto;">
    <h1 style="color: #2c3e50; margin-bottom: 20px;">Create New Bill</h1>
    ...
    <div class="form-container">
        <form method="POST" action="<?= base_url('bills/store') ?>">
            <?= csrf_field() ?>
            ...
            <div class="form-group">
                ...
                <input type="number" id="rate_per_unit" name="rate_per_unit" step="0.01" value="<?= old('rate_per_unit') ?>" placeholder="0.00" required>
            </div>

            <div class="form-group">
                <label for="total_amount">Total Amount (₱) <span style="color: red;">*</span></label>
                <input type="number" id="total_amount" name="total_amount" step="0.01" value="<?= old('total_amount') ?>" placeholder="0.00" required>
            </div>

            <div class="form-buttons">
                <button type="submit" class="btn btn-success">Create Bill</button>
                <a href="<?= base_url('bills') ?>" class="btn btn-secondary">Cancel</a>
            </div>
        </form>
    </div>
</div>
